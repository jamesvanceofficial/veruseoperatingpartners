...
export function ProposalShareLinkPanel({
  proposalId,
  initialToken,
  initialExpiresAt,
  initialActive,
  ...
  const url = token ? `${origin}/proposal/${token}` : null;

  async function handleGenerate() {
    setLoading(true);
    setError(null);
    try {
      const res = await fetch(`/api/proposals/${proposalId}/share-link`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ expiresInDays: expiresInDays === "never" ? null : Number(expiresInDays) }),
      });
  ...
  async function handleRevoke() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/proposals/${proposalId}/share-link/revoke`, { method: "POST" });
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
      ...
      {active && token ? (
        <>
          <div className="flex items-center gap-2">
            <code className="field-control flex-1 truncate px-3 py-2 text-[12px]">{url ?? `/proposal/${token}`}</code>
