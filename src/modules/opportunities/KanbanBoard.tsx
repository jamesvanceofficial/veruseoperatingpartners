        ...
        {PIPELINE_STAGES.map((stage) => {
          ...
          return (
            <div
              ...
              className="glass-panel flex w-72 shrink-0 flex-col gap-2 p-3"
            >
              <div className="flex items-center justify-between gap-2 px-1">
                <Badge tone={STAGE_TONE[stage]}>{STAGE_LABELS[stage]}</Badge>
                <span className="text-[11px] text-[var(--muted)]">{cards.length}</span>
              </div>
              <p className="px-1 font-tabular text-[11px] text-[var(--muted)]">{formatCurrency(stageValue)}</p>
              ...
              <div className="flex flex-col gap-2">
                {cards.map((card) => {
                  ...
                  return (
                    <Link
                      ...
                      className={cn(
                        "glass-panel row-hover-lift flex cursor-grab flex-col gap-1.5 p-3 text-left active:cursor-grabbing",
                ...
                {cards.length === 0 ? <p className="px-1 py-2 text-center text-[11px] text-[var(--muted)]">No opportunities</p> : null}
              </div>
            </div>
          );
        })}
